...
#region Signals
## Emitted after the machine enters [param to], for the initial state and every
## transition. Listen to react to state changes without polling [member current_state].
signal state_changed(to: FSMState)
#endregion
...
func _force_change_state(state: FSMState) -> void:
	...
	var changed: bool = _current_state != state
	_last_state = _current_state
	_current_state = state
	_current_state.enter()
	# Only announce real transitions; states that return self to "stay" re-enter
	# every frame and should not spam the signal.
	if changed:
